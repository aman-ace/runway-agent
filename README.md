...
and no JS, so it opens fine offline or as an email attachment. It's styled
like a paper ledger and stays light on purpose, regardless of the reader's
OS theme — a board memo shouldn't flip dark because of someone's system
settings.
...
```mermaid
flowchart TD
    csv["Bank CSV<br/>date, description, amount"] --> agent["agent.py — CLI"]

    agent --> burn["burn.py<br/>deterministic math, no model"]
    burn --> summary["monthly burn + runway"]
    burn --> variance["step-up / one-off / creep detection"]

    agent --> cat["categorize.py"]
    cat --> rules{"matched by<br/>regex rules?"}
    rules -->|"yes, ~100% of rows"| tagged["category + flow"]
    rules -->|"no, unmatched vendors"| model

    agent --> provider{"LLM_PROVIDER"}
    provider -->|gemini| model["LLM call<br/>Gemini or Ollama"]
    provider -->|ollama| model
    provider -->|"none / unavailable"| skip["skipped"]

    model -->|"JSON mode, temp 0.1"| tagged
    model -->|"free text, temp 0.6"| commentary["board commentary"]
    skip -.->|"templated fallback"| commentary

    summary --> report["report.py"]
    variance --> report
    tagged --> report
    commentary --> report

    report --> charts["charts.py<br/>inline SVG, no deps"]
    report --> md["Markdown report"]
    charts --> html["HTML report<br/>ledger design"]
    report --> html
```

The two things worth noticing in the diagram: the model sits off to the side
of `burn.py` entirely, never downstream of it, and `categorize.py` only calls
the model for the handful of vendor strings the regex table couldn't match.
